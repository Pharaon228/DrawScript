...
async function chooseBooster(page, boosterSelections, id) {
    ...
    if (!asked) {
        ...
        console.log("Выберете бустер (0-без бустера, 1-первого уровная, 2-второго уровня, 3-третьего уровня, 4-четвёртого уровня): ");
...
async function getBalanceAndChoices(page, id) {
    ...
    for (const choice of choices) {
        ...
        if ((purchases > 0) && (choice.level != 'без бустера')) {
            console.log(`Для ${id + 1} сессии Бустер ${choice.level} уровня доступен. Можно купить ${purchases} штук. Продолжительность работы: ${purchases * 3} минут.`);
        } else if (choice.level != 'без бустера') {
            console.log(`Для ${id + 1} сессии Бустер ${choice.level}сессии уровня не доступен. Недостаточно средств.`);
        }
    }
