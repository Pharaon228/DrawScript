async function checkAndPurchaseBooster(page, chosenBooster, clickCounter, id, boosterCheck) {
  ...
  if ((balance >= chosenBooster.cost) && (chosenBooster.level != "без бустера")) {
    if ((boosterCheck > 22) || (boosterCheck === 0)) {
      try {

        console.log("\x1b[32m%s\x1b[0m", "Активный бустер отсутствует. Покупаем бустер...");

        const shopButton = await page.waitForSelector('a.MenuItem-yi1zwm-1.AgRZi[href="/shop"]', { timeout: 5000 });
        if (shopButton) {
          await page.click('a.MenuItem-yi1zwm-1.AgRZi[href="/shop"]');
          await delay(1000);
        }
        const boosterLinkButton = await page.waitForSelector(chosenBooster.link, { timeout: 5000 });
        if (boosterLinkButton) {
          await page.click(chosenBooster.link);
        }

        const confirmButton = await page.waitForSelector(".BlackButtonStyled-sc-155f8n4-0.gvlQbP", { timeout: 5000 });
        if (confirmButton) {
        ...
        console.log(`Для сессии ${id + 1} за время работы было совершено ${clickCounter} нажатий.`);
...
async function getCurrentBalance(page) {
  try {
    await page.waitForSelector(".AnimatedNumberStyled-sc-98h1so-0.kSpFWd", { timeout: 5000 });
